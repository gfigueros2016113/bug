...
if (isset($_GET)) {
    if (isset($_GET["quest"])) {
        if ($_GET["quest"] == 'clientes_hace_1mes') {
            $mysql_query = "select distinct c.*, idC.Dias, sum(dp.precio*dp.cantidad) as total from clientes_online as c inner join (SELECT DISTINCT *, TIMESTAMPDIFF(DAY, fecha_generado, NOW()) AS Dias  FROM pedido_online WHERE MONTH(fecha_generado) = MONTH(DATE_SUB(NOW(), INTERVAL 1 MONTH)) AND YEAR(fecha_generado) = YEAR(NOW()) ORDER BY fecha_generado DESC) as idC on idC.id_cliente_online = c.id inner join detalle_pedido_online as dp on dp.id_pedido_online=idC.id WHERE idC.estado='confirmado' and c.id NOT IN(select c.id from clientes_online as c inner join (SELECT DISTINCT * FROM pedido_online WHERE MONTH(fecha_generado) = MONTH(NOW()) AND YEAR(fecha_generado) = YEAR(NOW()) ORDER BY fecha_generado DESC) as idC on idC.id_cliente_online = c.id) group by(c.nombre);";
            $resultado = mysqli_query($con, $mysql_query);
...
if (isset($_GET)) {
    if (isset($_GET["quest"])) {
        if ($_GET["quest"] == 'clientes_hace_2mes') {
            $mysql_query = "select distinct c.*, idC.Dias, sum(dp.precio*dp.cantidad) as total from clientes_online as c inner join (SELECT DISTINCT *, TIMESTAMPDIFF(DAY, fecha_generado, NOW()) AS Dias  FROM pedido_online WHERE MONTH(fecha_generado) = MONTH(DATE_SUB(NOW(), INTERVAL 2 MONTH)) AND YEAR(fecha_generado) = YEAR(NOW()) ORDER BY fecha_generado DESC) as idC on idC.id_cliente_online = c.id inner join detalle_pedido_online as dp on dp.id_pedido_online=idC.id WHERE idC.estado='confirmado' and c.id NOT IN(select c.id from clientes_online as c inner join (SELECT DISTINCT * FROM pedido_online WHERE MONTH(fecha_generado) = MONTH(NOW()) AND YEAR(fecha_generado) = YEAR(NOW()) ORDER BY fecha_generado DESC) as idC on idC.id_cliente_online = c.id) group by(c.nombre);";
            $resultado = mysqli_query($con, $mysql_query);
